...
interface ChatSidebarProps{
    ...
    onSendMessage: () => void
    onSuggestionClick: (suggestions: string) => void
}
...
function ChatSidebar({
  ...
  const chatSuggestions = [
    ...
    "What suggestion was I givevn during the discussion?",
              ...
              {chatSuggestions.map((suggestion, index) => (
                <button
                  ...
                  className={`w-4/5 rounded-lg p-4 border transition-colors text-center ${canChat
                    ? 'bg-primary/10 text-muted-foreground border-primary/20 hover:bg-primary/20'
                    : 'bg-muted/50t text-muted-foreground border-muted cursor-not-allowed'
                  }`}
                >
